...
| 9 | Trying to build a `demo_code/` reference folder directly (not your own project) fails with `make: *** No rule to make target 'all'` | Every `demo_code/` folder in this project is source-only, by design (Section 6): none of them are meant to be built on their own. Create the one wizard-based dashboard project Section 6 describes and copy that folder's `.c`/`.h` logic into it instead of trying to build the reference folder itself. | Easy |
| 10 | Build fails with `fatal error: fsl_XXX.h: No such file or directory` (e.g. `fsl_adc16.h`) after copying reference code into your wizard project | The wizard project only includes driver files for what you selected at creation (UART console), not every peripheral. Copy the missing driver's `.c`/`.h` pair from `SDK_2_2_0_FRDM-KL26Z/devices/MKL26Z4/drivers/` into your project's own `drivers/` folder, then refresh (F5) and rebuild. | Easy |
| 11 | `PRINTF("...%lu...", value)` or similar prints the letters `lu` literally, with no digits | Same root cause as P0's `%08lX` bug: your wizard project links Redlib, and its default `PRINTF_ADVANCED_ENABLE=0` silently drops support for the `l` length modifier. Drop the `l`; plain `%u` (for `uint32_t`) or `%X` prints the identical result on this 32-bit chip. | Easy |
| 12 | `PRINTF("...%f...")` or `%.2f` prints nothing at all where the number should be, no error | This project's build also has `PRINTF_FLOAT_ENABLE=0` (visible in the compiler flags in the Console), so floating-point format specifiers do not work at all, not even garbage output. Don't fix this by re-enabling float support; instead do the math in integer terms (e.g. compute millivolts, `(raw * 3300u) / 4095u`, instead of volts as a float) and print with `%u`. This is the same fixed-point habit P3 teaches deliberately later in the series, since this chip has no hardware floating-point unit. | Medium |
| 13 | Pressing the button labeled "SW1" on the board does nothing, but a different button works | The code's `BOARD_SW1_NAME` ("SW1", PTC3) doesn't always match this board's physical silkscreen; on some boards the button wired to PTC3 is printed as SW2. Press whichever physical button is actually wired to PTC3, regardless of what it's labeled, and it will work. Verified by hand on real hardware this session. | Easy |
